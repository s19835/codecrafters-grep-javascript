  ...
  while (i < pattern.length) {
    if (pattern[i] === '\\') {
      if (pattern[i + 1]) {
        ...
        throw new Error(`Invalid escape sequence at end of pattern: ${pattern}`);
      }
    } 
      ...
      if (j >= pattern.length) {
        throw new Error(`Unclosed group starting at index ${i}: ${pattern}`);
      }
      tokens.push(group.split('|'));
      i = j + 1;
    } 
    
    else if (pattern[i] === '+' || pattern[i] === '?') {
      if (tokens.length === 0) {
        throw new Error(`Invalid '${pattern[i]}' quantifier at start of pattern: ${pattern}`);
      } else {
...
function matchPattern(inputLine, pattern) {
  ...
  while (inputIndex < inputLine.length && tokenIndex < tokens.length) {
    if (Array.isArray(tokens[tokenIndex])) {
      // Handle group with `|`
      for (const subPattern of tokens[tokenIndex]) {
        if (matchPattern(inputLine.slice(inputIndex), subPattern)) {
          return true;
        }
      }
      return false;
